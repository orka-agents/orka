...
SCRIPT = Path(__file__).resolve().parents[2] / "skills" / "autoreview" / "scripts" / "autoreview"
...
class AutoreviewHardeningTests(unittest.TestCase):
    ...
    @unittest.skipIf(os.name == "nt", "POSIX shell wrapper behavior")
    def test_shell_harness_resolves_bare_path_invocation(self) -> None:
        trusted_python = Path(sys.executable).resolve()
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            repo = root / "repo"
            ...
            shutil.copy2(SCRIPT.with_name("test-review-harness"), wrapper)
            wrapper.chmod(0o755)
            harness.write_text("print('resolved harness')\n", encoding="utf-8")
            outside = root / "outside"
            outside.mkdir()
            env = os.environ.copy()
            env["PATH"] = os.pathsep.join(
                (str(scripts), str(trusted_python.parent), "/usr/bin", "/bin")
            )
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertIn("resolved harness", result.stdout)

    @unittest.skipIf(os.name == "nt", "POSIX shell wrapper behavior")
    def test_shell_harness_resolves_bare_cwd_invocation(self) -> None:
        trusted_python = Path(sys.executable).resolve()
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            ...
            shutil.copy2(SCRIPT.with_name("test-review-harness"), wrapper)
            wrapper.chmod(0o755)
            harness.write_text("print('cwd harness')\n", encoding="utf-8")
            env = os.environ.copy()
            env["PATH"] = os.pathsep.join(
                (str(trusted_python.parent), "/usr/bin", "/bin")
            )
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertIn("cwd harness", result.stdout)

    @unittest.skipIf(os.name == "nt", "POSIX shell wrapper behavior")
    def test_shell_harness_rejects_ambiguous_bare_invocation(self) -> None:
        trusted_python = Path(sys.executable).resolve()
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            cwd_scripts = root / "cwd"
            path_scripts = root / "path"
            cwd_scripts.mkdir()
            path_scripts.mkdir()
            for scripts, message in (
                (cwd_scripts, "cwd harness"),
                (path_scripts, "path harness"),
            ):
                wrapper = scripts / "test-review-harness"
                harness = scripts / "test-review-harness.py"
                shutil.copy2(SCRIPT.with_name("test-review-harness"), wrapper)
                wrapper.chmod(0o755)
                harness.write_text(f"print({message!r})\n", encoding="utf-8")
            env = os.environ.copy()
            env["PATH"] = os.pathsep.join(
                (str(path_scripts), str(trusted_python.parent), "/usr/bin", "/bin")
            )

            result = subprocess.run(
                ["/bin/sh", "test-review-harness"],
                cwd=cwd_scripts,
                env=env,
                text=True,
                capture_output=True,
            ...
            self.assertIn("Ambiguous test-review-harness invocation", result.stderr)
    ...
    def test_review_patch_allows_known_synthetic_fixture_literals(self) -> None:
        patch = (
            "diff --git a/fixtures.py b/fixtures.py\n"
            "--- a/fixtures.py\n"
            "+++ b/fixtures.py\n"
            "@@ -1,4 +0,0 @@\n"
            + fixture_text('\n"eulav esarhp terces kcom"=DROWSSAP-')
            + fixture_text('\n"eulav yek ipa kcom" = yek_ipa-')
            + fixture_text(
                "\nppa/tset.elpmaxe.bd@drowssap-kcom:moc.elpmaxe@resu//:sergtsop=LRU_ESABATAD-"
            )
            + fixture_text('\n"nekot-kcom" = nekot-')
        )
    ...
    def test_source_tree_snapshot_hashes_binary_and_untracked_tail_bytes(
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            untracked.write_bytes(b"\0" + b"a" * (limit - 1))
            before = self.helper["source_tree_snapshot"](repo)
    ...
    def test_source_file_fingerprint_rejects_oversized_untracked_files(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            path = Path(tempdir) / "generated.bin"
            limit = self.helper["MAX_BUNDLE_TEXT_BYTES"]
            path.write_bytes(b"a" * (limit * 8))

            with mock.patch.object(
                self.helper["os"],
                "read",
                wraps=os.read,
            ) as read_mock, self.assertRaisesRegex(
                SystemExit,
                "refusing to fingerprint oversized file",
            ):
                self.helper["source_file_fingerprint"](
                    path,
    ...
    def test_source_file_fingerprint_rejects_growth_before_open(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            path = Path(tempdir) / "generated.bin"
            limit = self.helper["MAX_BUNDLE_TEXT_BYTES"]
            ...
            def grow_then_open(target: object, flags: int) -> int:
    ...
    def test_source_file_fingerprint_bounds_growth_during_read(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            path = Path(tempdir) / "generated.bin"
            limit = self.helper["MAX_BUNDLE_TEXT_BYTES"]
            path.write_bytes(b"a")
            ...
            def append_then_read(descriptor: int, count: int) -> bytes:
                nonlocal appended
                if not appended:
                    appended = True
                    with path.open("ab") as stream:
                        stream.write(b"b" * (limit + 1))
                return real_read(descriptor, count)

            with mock.patch.object(
                self.helper["os"],
                "read",
                side_effect=append_then_read,
            ) as read_mock, self.assertRaisesRegex(
                SystemExit,
                "refusing to fingerprint oversized file",
            ):
            ...
            self.assertLessEqual(read_mock.call_count, 2)
