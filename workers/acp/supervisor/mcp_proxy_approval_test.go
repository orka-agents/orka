...
func TestMCPProxyBrokeredApprovalWaitsForControllerResult(t *testing.T) {
	...
	promptTrace := trace.NewSpanContext(trace.SpanContextConfig{
		TraceID: trace.TraceID{1}, SpanID: trace.SpanID{2}, TraceFlags: trace.FlagsSampled,
	})
	if err := session.activate(trace.ContextWithSpanContext(t.Context(), promptTrace), authorization, lease, now); err != nil {
		t.Fatal(err)
	}
	...
	if !trace.SpanContextFromContext(pending.ctx).Equal(promptTrace) {
		t.Fatal("approval wait lost the admitted prompt trace")
	}
...
func assertMCPApprovalIndependentSession(t *testing.T, session *mcpProxySession) {
	...
	other.mu.Unlock()
	if err := other.activate(t.Context(), otherAuthorization, otherLease, time.Now().UTC()); err != nil {
		t.Fatal(err)
	}
