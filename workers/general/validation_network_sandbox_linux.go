...
func installValidationNetworkSandbox() error {
	...
	filters := []unix.SockFilter{
		...
		validationSandboxStatement(unix.BPF_RET|unix.BPF_K, unix.SECCOMP_RET_KILL_PROCESS),
		validationSandboxJump(unix.BPF_JMP|unix.BPF_JEQ|unix.BPF_K, uint32(unix.SYS_SOCKET), 0, 4),
		validationSandboxStatement(unix.BPF_LD|unix.BPF_W|unix.BPF_ABS, seccompDataArg0Offset),
		validationSandboxJump(unix.BPF_JMP|unix.BPF_JEQ|unix.BPF_K, unix.AF_INET, 1, 0),
		validationSandboxJump(unix.BPF_JMP|unix.BPF_JEQ|unix.BPF_K, unix.AF_INET6, 0, 1),
