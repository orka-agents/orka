#!/usr/bin/env bash
# Validates the harness v1/v2 coexistence bridge CRDs against a live cluster:
# both-baseline object acceptance, discriminator CEL, contract immutability,
# legacy-surface ratcheting, and Task binding/quarantine status invariants.
# The bridge CRDs from config/crd/bases must already be applied and Established.
#
# Usage: KUBECTL="kubectl" scripts/coexistence-bridge-crd-validate.sh
set -euo pipefail

KUBECTL=${KUBECTL:-kubectl}
NAMESPACE=${NAMESPACE:-bridge-crd-validate}
...
BINDING=$(cat <<JSON
{"schemaVersion":1,"mode":"execute","contractVersion":"orka.harness.v1","backend":"harness-wrapper","provenance":"legacy-adopted","bindingDigest":"${ZERO_DIGEST}","task":{"namespaceUID":"ns-uid","uid":"task-uid","boundSpecGeneration":1},"snapshot":{"id":"task-uid/${ZERO_DIGEST}","digest":"${ZERO_DIGEST}","schemaVersion":1},"boundAt":"2026-08-05T00:00:00Z"}
JSON
)
...
expect_status_patch reject "a bound Task cannot also record quarantine" \
  tasks.core.orka.ai/bound-task "{\"status\":{\"agentExecutionQuarantine\":{\"schemaVersion\":1,\"reason\":\"MixedV1V2Evidence\",\"migrationInventoryID\":\"inv-1\",\"recordedAt\":\"2026-08-05T00:00:00Z\"}}}" "quarantined Task cannot hold an execution binding"
...
{"schemaVersion":1,"mode":"execute","contractVersion":"orka.harness.v1","backend":"runtime-pool","provenance":"newly-bound","bindingDigest":"${ZERO_DIGEST}","task":{"namespaceUID":"a","uid":"b","boundSpecGeneration":1},"snapshot":{"id":"b/${ZERO_DIGEST}","digest":"${ZERO_DIGEST}","schemaVersion":1},"boundAt":"2026-08-05T00:00:00Z"}
JSON
)
...
echo "== AgentExecutionControl / Policy / Adjudication =="

expect_reject "AgentExecutionControl must be the cluster singleton" "singleton named cluster" <<EOF
apiVersion: core.orka.ai/v1alpha1
kind: AgentExecutionControl
metadata: {name: not-cluster, namespace: ${NAMESPACE}}
spec:
  backends:
    v1: {desiredMode: disabled}
    v2: {desiredMode: enabled}
EOF

expect_accept "AgentExecutionControl singleton is accepted" <<EOF
apiVersion: core.orka.ai/v1alpha1
kind: AgentExecutionControl
...
expect_status_patch accept "backend mode status writes with revision" \
  agentexecutioncontrols.core.orka.ai/cluster '{"status":{"backends":{"v1":{"effectiveMode":"disabled","modeRevision":3},"v2":{"effectiveMode":"enabled","modeRevision":5}}}}'

expect_status_patch reject "modeRevision cannot decrease" \
  agentexecutioncontrols.core.orka.ai/cluster '{"status":{"backends":{"v1":{"effectiveMode":"disabled","modeRevision":2},"v2":{"effectiveMode":"enabled","modeRevision":5}}}}' "must not decrease"

expect_status_patch reject "admissionClosedAt requires drain-only or disabled" \
  agentexecutioncontrols.core.orka.ai/cluster '{"status":{"backends":{"v1":{"effectiveMode":"disabled","modeRevision":3},"v2":{"effectiveMode":"enabled","modeRevision":6,"admissionClosedAt":"2026-08-05T00:00:00Z"}}}}' "admissionClosedAt requires"

expect_reject "policy cannot drop mandatory fail-closed floors" "mandatory fail-closed floors" <<EOF
apiVersion: core.orka.ai/v1alpha1
kind: AgentExecutionPolicy
metadata: {name: weak-policy, namespace: ${NAMESPACE}}
spec:
  allowNewV1Bindings: true
  retryEligibility: none
  prohibitedFields: [workspace-credentials, forge-credentials, direct-publication, observed-write-tools]
  networkIsolationProfile: default-deny
EOF

expect_reject "policy cannot allow new v1 OpenCode bindings" "sealed-inventory" <<EOF
apiVersion: core.orka.ai/v1alpha1
kind: AgentExecutionPolicy
metadata: {name: opencode-policy, namespace: ${NAMESPACE}}
spec:
  allowNewV1Bindings: true
  allowedBuiltInRuntimeTypes: [opencode]
  retryEligibility: none
  prohibitedFields: [workspace-credentials, forge-credentials, direct-publication, transaction-tokens]
  networkIsolationProfile: default-deny
EOF

expect_accept "compatibility policy with floors is accepted" <<EOF
apiVersion: core.orka.ai/v1alpha1
kind: AgentExecutionPolicy
metadata: {name: compatibility, namespace: ${NAMESPACE}}
spec:
  allowNewV1Bindings: true
  allowedBuiltInRuntimeTypes: [claude]
  allowPublicReadOnlyWorkspaces: true
  retryEligibility: duplicate-safe-only
  prohibitedFields: [workspace-credentials, forge-credentials, direct-publication, transaction-tokens]
  networkIsolationProfile: per-trust-domain
EOF

expect_reject "adjudication requires quarantine or blocked-state evidence" "quarantine or blocked-state evidence" <<EOF
apiVersion: core.orka.ai/v1alpha1
kind: AgentExecutionAdjudication
metadata: {name: no-evidence, namespace: ${NAMESPACE}}
spec:
  taskRef: {name: bound-task, uid: task-uid}
  expectedState: {subjectResourceVersion: "1", evidenceClosureWatermark: "${ZERO_DIGEST}"}
  action: CleanupBoth
  evidenceDigests: ["${ZERO_DIGEST}"]
  justification: verified
  requestedBy: admin@example.com
EOF

expect_accept "adjudication with evidence is accepted" <<EOF
apiVersion: core.orka.ai/v1alpha1
kind: AgentExecutionAdjudication
metadata: {name: cleanup-both, namespace: ${NAMESPACE}}
spec:
  taskRef: {name: bound-task, uid: task-uid}
  expectedState: {subjectResourceVersion: "1", evidenceClosureWatermark: "${ZERO_DIGEST}"}
  quarantineDigest: "${ZERO_DIGEST}"
  action: CleanupBoth
  evidenceDigests: ["${ZERO_DIGEST}"]
  justification: verified
  requestedBy: admin@example.com
...
expect_reject "adjudication spec is immutable" "immutable" <<EOF
apiVersion: core.orka.ai/v1alpha1
kind: AgentExecutionAdjudication
metadata: {name: cleanup-both, namespace: ${NAMESPACE}}
spec:
  taskRef: {name: bound-task, uid: task-uid}
  expectedState: {subjectResourceVersion: "2", evidenceClosureWatermark: "${ZERO_DIGEST}"}
  quarantineDigest: "${ZERO_DIGEST}"
  action: CleanupBoth
  evidenceDigests: ["${ZERO_DIGEST}"]
  justification: verified
...
if [[ ${FAILURES} -gt 0 ]]; then
  echo "bridge CRD validation FAILED: ${FAILURES} case(s)" >&2
  exit 1
fi
echo "bridge CRD validation passed"
