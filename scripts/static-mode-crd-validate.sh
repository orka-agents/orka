...
# Prompt content is validated while freezing the Agent configuration; CRD
# admission must allow both supported OpenCode prompt sources.
expect_accept "v2 OpenCode Agent with inline systemPrompt is accepted" <<EOF
apiVersion: core.orka.ai/v1alpha1
kind: Agent
...
spec:
  ...
  systemPrompt: {inline: "Cite source URLs and publication dates."}
EOF

expect_accept "v2 OpenCode Agent with ConfigMap systemPrompt is accepted" <<EOF
apiVersion: core.orka.ai/v1alpha1
kind: Agent
metadata: {name: v2-opencode-configmap, namespace: ${NAMESPACE}}
spec:
  runtime: {type: opencode, contractVersion: orka.harness.v2}
  model: {name: engine/gpt-5.2, contextWindow: 128000, maxTokens: 8192}
  systemPrompt: {configMapRef: {name: opencode-prompt, key: instructions}}
EOF
