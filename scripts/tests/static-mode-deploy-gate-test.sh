...
grep -F 'RUN_AGENT_EXECUTION_SNAPSHOT_KEY_FILE ?=' "${root}/Makefile" >/dev/null
...
grep -F -- '--agent-execution-snapshot-key-file="$(RUN_AGENT_EXECUTION_SNAPSHOT_KEY_FILE)"' "${root}/Makefile" >/dev/null
