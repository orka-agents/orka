...
- OpenTelemetry GenAI constants are hand-rolled in `internal/tracing/genai`; telemetry is enabled with `--enable-telemetry`/`--enable-tracing`, workers honor `ORKA_ENABLE_TELEMETRY` or `OTEL_EXPORTER_OTLP_ENDPOINT`, and prompt/completion content capture remains default-off/fail-closed
