...
# Only known lifecycle codes may appear verbatim in cancellation diagnostics.
# Unexpected reasons and free-form messages are retained only as hashes.
CANCELLATION_DIAGNOSTIC_CODES = TERMINAL | {
    "Pending", "Running", "Finalizing", "Scheduled", "Ready", "Complete", "True", "False", "Unknown",
    "NotRequested", "VerifiedExact", "DeliveredSuperseded", "ReadValidated", "NoChange",
    "CancelledBeforePublish", "ReadOnlyWorkspaceModified", "DeliveryConflict", "CredentialBlocked",
    "PublicationOutcomeUnknown", "AtCapacity", "PromptFailed", "TaskTimeout", "RuntimeLost",
    "ExecutionEventPersistenceFailed", "ControllerRestartRecovered", "RestoreIdentityChanged",
    "InvalidRuntimeProfile", "InvalidWorkspace", "WorkspaceFailed", "WorkspaceUnsupported",
    "WorkspaceValidationFailed", "PublicationFailed", "CancellationRequestedAfterPublish", "RestoreEvidencePreserved",
}
...
def validate_cancellation_receipt(task, approval, before, after):
    metadata, binding = task["metadata"], approval["binding"]
    require(metadata.get("deletionTimestamp") and metadata["uid"] == approval["taskUID"],
            "cancellation evidence must belong to the original deleting Task")
    expected = {"kind": "acp-mcp-tool", "identityNamespace": metadata["namespace"],
                "aggregateId": binding["runtimeSessionUID"],
                "requestDigest": binding["requestDigest"]}
    require(before["metadata"].get("uid") and after["metadata"].get("uid") == before["metadata"]["uid"] and
            after.get("spec") == before.get("spec") and
            all(after["spec"].get(key) == value for key, value in expected.items()) and
            "sha256:" + hashlib.sha256(after["spec"].get("operationId", "").encode()).hexdigest() == binding["operationIDDigest"],
            "cancellation changed the original action receipt identity")
    status = after.get("status", {})
    require(before.get("status", {}).get("state") == "Pending" and status.get("state") == "Failed" and
            status.get("attempts", 0) == 0 and not status.get("leaseOwner") and not status.get("leaseExpiresAt"),
            "cancelled action was claimed or lacks a definitive denial receipt")
    receipt = status.get("response", {})
    require(receipt.get("isError") is True and receipt.get("code") == "approval_cancelled" and
            receipt.get("approvalID") == approval["id"], "cancelled action lost its original denial receipt")
    # This denial receipt contains ASCII strings and a boolean, so the compact
    # sorted encoding agrees with the controller's canonical JSON digest.
    digest = "sha256:" + hashlib.sha256(json.dumps(receipt, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    require(status.get("responseDigest") == digest, "cancelled action receipt failed integrity verification")
    task_status = task.get("status", {})
    outcome = task_status.get("execution", {}).get("outcome")
    require((outcome == "Cancelled" and task_status.get("phase") == "Cancelled") or
            (outcome == "Succeeded" and task_status.get("phase") == "Succeeded" and
             task_status.get("delivery", {}).get("outcome") == "ReadValidated"),
            "cancellation neither stopped the prompt nor completed with validated delivery")
    return {"name": after["metadata"]["name"], "uid": after["metadata"]["uid"],
            "state": status["state"], "attempts": 0, "responseDigest": digest,
            "executionOutcome": "not_started", "code": receipt["code"]}


def cancellation_diagnostics(task):
    """Keep Task lifecycle evidence without provider text, inputs, or credentials."""
    def code(value):
        return value if isinstance(value, str) and value in CANCELLATION_DIAGNOSTIC_CODES else None

    def digest(value):
        return "sha256:" + hashlib.sha256(value.encode()).hexdigest() if isinstance(value, str) and value else None

    def timestamp(value):
        if not isinstance(value, str) or not re.fullmatch(
            r"\d{4}-\d{2}-\d{2}T[0-2]\d:[0-5]\d:[0-5]\d(?:\.\d{1,9})?"
    ...
    def lifecycle(value):
        return {"outcome": code(value.get("outcome")), "reason": code(value.get("reason")),
                "reasonSHA256": digest(value.get("reason")), "messageSHA256": digest(value.get("message")),
                "lastTransitionTime": timestamp(value.get("lastTransitionTime"))}

    metadata, status = task["metadata"], task.get("status", {})
    return {
        "observedAt": datetime.now(timezone.utc).isoformat(),
        "metadata": {**{field: metadata.get(field) for field in ("namespace", "name", "uid", "generation", "resourceVersion")},
                     **{field: timestamp(metadata.get(field)) for field in ("creationTimestamp", "deletionTimestamp")}},
        "phase": code(status.get("phase")), "messageSHA256": digest(status.get("message")),
        "startTime": timestamp(status.get("startTime")), "completionTime": timestamp(status.get("completionTime")),
        "execution": lifecycle(status.get("execution") or {}),
        "delivery": lifecycle(status.get("delivery") or {}),
        "conditions": [{"type": code(value.get("type")), "typeSHA256": digest(value.get("type")),
                        "status": code(value.get("status")), "reason": code(value.get("reason")),
                        "reasonSHA256": digest(value.get("reason")), "messageSHA256": digest(value.get("message")),
                        "lastTransitionTime": timestamp(value.get("lastTransitionTime"))}
...
class Acceptance:
    ...
    def observe(self, name, enabled):
        def update():
            metadata = self.kube_json("get", "task", name, "-o", "json")["metadata"]
            require(metadata["uid"] == self.owned_tasks[name], "refusing to update a replaced Task")
            finalizers = metadata.get("finalizers", [])
            if (FINALIZER in finalizers) == enabled:
                return True
            ...
            try:
                ...
                # Controller cleanup can change the version or its finalizers
                # between GET and PATCH. Retry only after observing that change.
                current = self.kube_json("get", "task", name, "-o", "json")["metadata"]
                require(current["uid"] == self.owned_tasks[name], "refusing to update a replaced Task")
                if current["resourceVersion"] == metadata["resourceVersion"]:
                    raise
                return None
            return True
        self.wait("observer finalizer update", update, 30)

    def approval_effect(self, name, approval):
        listed = self.kube_json("get", "externaleffects", "-l",
                                "core.orka.ai/task-uid=" + self.owned_tasks[name], "-o", "json")
        binding = approval["binding"]
        matches = [value for value in listed["items"] if value["spec"].get("kind") == "acp-mcp-tool" and
                   value["spec"].get("aggregateId") == binding["runtimeSessionUID"] and
                   "sha256:" + hashlib.sha256(value["spec"].get("operationId", "").encode()).hexdigest() == binding["operationIDDigest"]]
        require(len(matches) == 1, "original approval must have exactly one durable action record")
        return matches[0]
    ...
    def run_case(self, scenario):
        ...
        if scenario == "approve":
            ...
            cancelled_effect = self.approval_effect(name, original)
            ...
            def authority_lost():
                ...
                restarted = bool(boot) and "sha256:" + hashlib.sha256(boot.encode()).hexdigest() != original["binding"]["supervisorBootIDDigest"]
        ...
        if scenario == "cancel":
            # Save the original Task's terminal reason before any cancellation
            # assertion can fail or cleanup can reclaim the remaining records.
            self.report.setdefault("cancellationDiagnostics", []).append(cancellation_diagnostics(task))
            self.save()
            # Task cleanup can erase the approval timeline and result before
            # removing our observer finalizer. Check the durable denial below;
            # a late decision may correctly find no remaining approval event.
            status, _ = self.decision(name, original, "approve", (404, 409, 410))
            after = self.kube_json("get", "externaleffect", cancelled_effect["metadata"]["name"], "-o", "json")
            cancellation_evidence = {"lateDecisionHTTPStatus": status,
                "cancellationReceipt": validate_cancellation_receipt(task, original, cancelled_effect, after)}
        elif scenario != "recovery":
        ...
        if scenario == "cancel":
            ...
            if task["status"]["execution"]["outcome"] == "Succeeded":
                result_status, result = self.api("tasks/" + name + "/result", allowed=(200, 404))
                if result_status == 200:
                    require("approval_cancelled" in result.get("result", ""), "completed Task lost its cancellation result")
            if final_approval is not None:
                require(final_approval["id"] == original["id"] and final_approval.get("status") == "cancelled" and
                        final_approval.get("executionOutcome") == "not_started" and
                        final_approval.get("executionReason") == "approval_cancelled",
                        "cancelled review lost its unexecuted denial")
                if result_status == 200:
        ...
        self.report["checks"].append({
            ...
            "approvalStatus": final_approval["status"] if final_approval else None,
            "executionOutcome": "not_started" if scenario == "cancel" else final_approval.get("executionOutcome"),
