...
For an automated local run, use `bash scripts/human-approval-v2-e2e.sh` from the
repository root. The [CI gate](../../.github/workflows/human-approval-v2-e2e.yml)
builds pinned real adapters in a disposable kindctl cluster and checks both
approval lifecycles, reviewer permissions, and runtime/controller crashes.
It uses deterministic model and Azure transport fixtures. See the
[test setup](../../scripts/fixtures/human-approval-v2/README.md) for prerequisites
and evidence files. Azure-hosted qualification remains a separate run.
