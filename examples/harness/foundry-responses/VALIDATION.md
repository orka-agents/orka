...
| Turn admission, cancel identity, and endpoint safety | Focused adapter tests cover bounded consumed-turn tombstones, malformed/mismatched cancel rejection before mutation, and empty-hostname endpoint rejection; `validate.sh` also checks the live-smoke shell preflight. |
