        ...
        <>
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              <div><span className="text-muted-foreground">URL:</span> {tool.spec.http.url}</div>
              <div><span className="text-muted-foreground">Method:</span> {tool.spec.http.method ?? 'POST'}</div>
              {tool.spec.http.timeout && <div><span className="text-muted-foreground">Timeout:</span> {tool.spec.http.timeout}</div>}
              {tool.spec.http.authInject && <div><span className="text-muted-foreground">Auth Inject:</span> {tool.spec.http.authInject}</div>}
              {tool.spec.http.headers && Object.keys(tool.spec.http.headers).length > 0 && (
                <div>
                  <span className="text-muted-foreground">Headers:</span>
                  <pre className="mt-1 rounded-md bg-muted p-2 text-xs">{JSON.stringify(tool.spec.http.headers, null, 2)}</pre>
                </div>
              )}
            </CardContent>
          </Card>
