...
describe('RuntimeRegistry', () => {
  ...
  it('shows an error for a runtime with an unsupported contract', async () => {
    const unsupported = externalRuntime()
    server.use(
      http.get('/api/v1/agent-runtimes', () => HttpResponse.json({
        items: [{ ...unsupported, spec: { ...unsupported.spec, contractVersion: 'orka.harness.v1' } }],
        metadata: {},
      })),
    )
    const user = userEvent.setup()
    ...
    await waitFor(() => expect(screen.getByText('Could not load agentruntime')).toBeInTheDocument())
    expect(screen.queryByText('external-codex')).not.toBeInTheDocument()
  })
})
