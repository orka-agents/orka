...
- runtime.type and model.provider are MUTUALLY EXCLUSIVE on the same Agent.
  - For coders/reviewers that need a workspace (git, shell): set
    ...
    maxTokens. OpenCode accepts an optional bounded literal systemPrompt; do not
    use environment/file substitutions in it. OMIT runtime.secretRef/secretRef
    because credentials come from the controller proxy.
