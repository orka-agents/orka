...
- When a child Task fails, ALWAYS fetch_task_output AND check Status.Message for these signals:
  ...
  - "provider_upstream_error" → the worker reached the LLM provider but the provider itself failed (rate limit, quota, outage, or an unreachable endpoint). This is an infrastructure fault, NOT a prompt, code, or Agent-shape bug. Retry the SAME task shape; if it keeps failing, report the blocker. Do NOT rewrite the prompt or recreate the Agent.
