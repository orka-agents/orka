...
func setupTestHandlers() (*Handlers, *fiber.App) {
	...
	handlers := NewHandlers(fakeClient, nil, "", ss, ss, nil)
...
func TestHandlers_CreateTask_NamespaceScoped(t *testing.T) {
	...
	_ = corev1alpha1.AddToScheme(scheme)
	fakeClient := fake.NewClientBuilder().WithScheme(scheme).Build()
	handlers := NewHandlers(fakeClient, nil, "allowed-ns", nil, nil, nil)
...
func TestNewHandlers(t *testing.T) {
	...
	handlers := NewHandlers(fakeClient, nil, "test-ns", nil, nil, nil)
	if handlers == nil {
...
func setupTestHandlersWithSessionManager() (*Handlers, *fiber.App, *sqlite.Store) {
	...
	handlers := NewHandlers(fakeClient, sm, "", ss, ss, nil)
...
func TestHandlers_ListSessions_WatchNamespace(t *testing.T) {
	...
	ss := sqlite.NewStore(db, ":memory:")
	sm := controller.NewSessionManager(ss)
	handlers := NewHandlers(fakeClient, sm, "watched-ns", ss, ss, nil)
...
func TestHandlers_GetTaskLogs_WatchNamespace(t *testing.T) {
	...
	handlers := NewHandlers(fakeClient, nil, "watched-ns", nil, nil, nil)
...
func TestHandlers_GetTaskResult_WatchNamespace(t *testing.T) {
	...
	handlers := NewHandlers(fakeClient, nil, "watched-ns", ss, ss, nil)
