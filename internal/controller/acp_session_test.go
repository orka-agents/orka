...
func TestPrepareRuntimeWorkspaceDoesNotCollapseMissingSessionIdentity(t *testing.T) {
	...
	first, err := dispatcher.prepareRuntimeWorkspace(context.Background(), firstTask, store.ControllerEpochFence{}, &acpTaskSession{}, time.Now().UTC())
	if err != nil {
		t.Fatal(err)
	}
	second, err := dispatcher.prepareRuntimeWorkspace(context.Background(), secondTask, store.ControllerEpochFence{}, &acpTaskSession{}, time.Now().UTC())
	if err != nil {
		t.Fatal(err)
	}
