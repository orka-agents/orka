...
func TestMCPApprovalSecretStorageFailuresAreUnavailable(t *testing.T) {
	for _, verb := range []string{"create", "get"} {
		for _, test := range []struct {
			name string
			err  error
		}{
			{name: "unavailable", err: apierrors.NewServiceUnavailable("simulated Secret API outage")},
			{name: "forbidden", err: apierrors.NewForbidden(schema.GroupResource{Resource: "secrets"}, "approval", errors.New("simulated access denial"))},
			{name: "timeout", err: apierrors.NewTimeoutError("simulated Secret API timeout", 1)},
		} {
			t.Run(verb+"/"+test.name, func(t *testing.T) {
				f := newMCPApprovalFixture(t)
				f.broker.ApprovalSecrets.(*k8sfake.Clientset).PrependReactor(verb, "secrets", func(k8stesting.Action) (bool, runtime.Object, error) {
					return true, nil, test.err
				})
				response := performMCPBrokerCall(t, f.broker, f.request, strings.Repeat("b", 32), []byte(strings.Repeat("c", 32)))
				if response.Code != http.StatusServiceUnavailable || f.count.Load() != 0 {
					t.Fatalf("storage failure: status=%d executions=%d", response.Code, f.count.Load())
				}
				if strings.Contains(response.Body.String(), "simulated") {
					t.Fatal("response exposed the backend error")
				}
				listed, err := approvals.ListEvents(t.Context(), f.events, f.request.Namespace, "approval-task")
				if err != nil || len(listed) != 0 {
					t.Fatalf("storage failure recorded approval events: count=%d err=%v", len(listed), err)
				}
			})
		}
	}
...
type approvalReserveErrorStore struct {
	store.ExternalEffectStore
	err error
}

func (s approvalReserveErrorStore) ReserveExternalEffect(context.Context, store.ReserveExternalEffectRequest) (*store.ExternalEffect, error) {
	return nil, s.err
}

func TestMCPApprovalReservationFailuresDistinguishConflicts(t *testing.T) {
	for _, test := range []struct {
		name       string
		err        error
		wantStatus int
	}{
		{name: "unavailable", err: errors.New("simulated effect store outage"), wantStatus: http.StatusServiceUnavailable},
		{name: "timeout", err: context.DeadlineExceeded, wantStatus: http.StatusServiceUnavailable},
		{name: "conflict", err: store.ConflictErrorf("simulated reservation conflict"), wantStatus: http.StatusConflict},
	} {
		t.Run(test.name, func(t *testing.T) {
			f := newMCPApprovalFixture(t)
			f.broker.Effects = approvalReserveErrorStore{ExternalEffectStore: f.broker.Effects, err: test.err}
			response := performMCPBrokerCall(t, f.broker, f.request, strings.Repeat("b", 32), []byte(strings.Repeat("c", 32)))
			if response.Code != test.wantStatus || f.count.Load() != 0 {
				t.Fatalf("reservation failure: status=%d want=%d executions=%d", response.Code, test.wantStatus, f.count.Load())
			}
			if strings.Contains(response.Body.String(), "simulated") {
				t.Fatal("response exposed the backend error")
			}
			listed, err := approvals.ListEvents(t.Context(), f.events, f.request.Namespace, "approval-task")
			if err != nil || len(listed) != 0 {
				t.Fatalf("reservation failure recorded approval events: count=%d err=%v", len(listed), err)
			}
		})
	}
}
...
func TestMCPApprovalChangedInputsCannotBorrowDecision(t *testing.T) {
	...
	for _, test := range []struct {
		name       string
		mutate     func(*harnessv2.MCPBrokerCallRequest)
		wantStatus int
	}{
		{name: "arguments", mutate: func(r *harnessv2.MCPBrokerCallRequest) { r.Call.Arguments = json.RawMessage(`{"value":"different"}`) }, wantStatus: http.StatusConflict},
		{name: "operation", mutate: func(r *harnessv2.MCPBrokerCallRequest) { r.Metadata.OperationID = "different-operation" }, wantStatus: http.StatusConflict},
		{name: "inconsistent fence", mutate: func(r *harnessv2.MCPBrokerCallRequest) { r.Metadata.Fence.RuntimeSessionGeneration++ }, wantStatus: http.StatusBadRequest},
	} {
		t.Run(test.name, func(t *testing.T) {
			changed := f.request
			test.mutate(&changed)
			...
			if response.Code != test.wantStatus || f.count.Load() != 1 {
				t.Fatalf("altered call: status=%d want=%d executions=%d", response.Code, test.wantStatus, f.count.Load())
			}
		})
	}
}
...
func TestMCPApprovalConcurrentDeliveryJoinsInFlightExecution(t *testing.T) {
	f := newMCPApprovalFixture(t)
	...
	f.broker.Executor = ACPMCPToolExecutorFunc(func(ctx context.Context, request harnessv2.MCPBrokerCallRequest, tool harnessv2.MCPToolDescriptor) (json.RawMessage, error) {
		result, err := executor.ExecuteACPMCPTool(ctx, request, tool)
		started <- struct{}{}
		select {
		case <-release:
			return result, err
	...
	original := f.start(f.request)
	pending := f.pending()
	f.decide(pending.ID, events.ExecutionEventTypeApprovalApproved)
	select {
	case <-started:
	case <-time.After(5 * time.Second):
		t.Fatal("approved action did not start")
	}
	// The executor is blocked before its receipt is saved. Discard reads from
	// the original approval wait, then prove the duplicate polls that live lease.
	select {
	case <-reads:
	default:
	}
	...
	case <-reads:
	case <-time.After(5 * time.Second):
		t.Fatal("duplicate delivery did not wait for the in-flight action")
	}
	select {
	case <-duplicate:
		t.Fatal("duplicate delivery returned before the execution receipt existed")
	default:
	}
	if f.count.Load() != 1 {
		t.Fatal("duplicate delivery repeated the in-flight action")
	}
	unblock()
	result := awaitMCPApprovalResult(t, original)
	replay := awaitMCPApprovalResult(t, duplicate)
	if result.IsError || result.Replayed || replay.IsError || !replay.Replayed ||
		string(result.Result) != `{"workOrder":"simulated-1"}` || string(replay.Result) != string(result.Result) || f.count.Load() != 1 {
		t.Fatal("concurrent delivery did not replay the original execution result exactly once")
	}
	f.reopen()
	durable := awaitMCPApprovalResult(t, f.start(f.request))
	if !durable.Replayed || string(durable.Result) != string(result.Result) || f.count.Load() != 1 {
		t.Fatal("concurrent delivery lost its durable execution receipt")
	}
}
