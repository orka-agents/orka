...
const usageCohortQuery = `WITH
 selection(namespace, namespace_uid, as_of, from_at, until_at, repository, kind, model) AS (VALUES (?, ?, ?, ?, ?, ?, ?, ?)),
 candidate_works AS (
 ...
 selected_tasks AS (
   ...
   WHERE t.namespace = f.namespace AND t.started_at <= f.as_of
 ...
 selected_counters AS (
   ...
   WHERE o.namespace = f.namespace AND o.observed_at <= f.as_of AND f.repository = '' AND f.kind = ''
     AND (f.namespace_uid IS NULL OR json_extract(o.data, '$.namespaceUID') = f.namespace_uid)
...
func loadUsageNamespace(ctx context.Context, db taskDataExecutor, namespace string, filter store.UsageFilter, result *store.UsageData) error {
	...
	rows, err := db.QueryContext(ctx, usageCohortQuery, namespace, namespaceUID, filter.AsOf.UnixNano(), from, until, strings.ToLower(filter.Repository), filter.Kind, filter.Model)
	if err != nil {
		return err
	}
