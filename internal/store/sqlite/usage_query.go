...
const usageCohortQuery = `WITH
 selection(namespace, as_of, from_at, until_at, repository, kind, model) AS (VALUES (?, ?, ?, ?, ?, ?, ?)),
 candidate_works AS (
...
func loadUsageNamespace(ctx context.Context, db taskDataExecutor, namespace string, filter store.UsageFilter, result *store.UsageData) error {
	...
	rows, err := db.QueryContext(ctx, usageCohortQuery, namespace, filter.AsOf.UnixNano(), from, until, strings.ToLower(filter.Repository), filter.Kind, filter.Model)
	if err != nil {
		return err
	}
