...
func validateGatewayMessageEnqueue(request store.GatewayMessageEnqueue) error {
	for name, value := range map[string]string{
		"namespace name": request.Namespace, "namespace UID": request.NamespaceUID, "event ID": request.EventID,
		"task name": request.TaskName, "task UID": request.TaskUID, "request ID": request.RequestID,
	} {
...
func enqueueGatewayMessageTx(ctx context.Context, tx *sql.Tx, request store.GatewayMessageEnqueue) (*store.GatewayDelivery, bool, error) {
	...
	// Length-delimited identity includes the event and Task incarnations, never
	// message text, timestamps or mutable controller policy.
	identity, err := json.Marshal([]string{request.Namespace, request.NamespaceUID, event.ID, event.TaskUID, request.RequestID})
	if err != nil {
		return nil, false, err
	}
	digest := sha256.Sum256(identity)
	id := "gdm-" + hex.EncodeToString(digest[:])
