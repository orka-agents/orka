...
- The ACP core path rejects provider-backed `Task.spec.execution.workspace`. Any future agent-sandbox/Substrate integration must use prompt/operation-scoped capabilities and must not stage raw transaction tokens into retained workspaces or provider child environments.
