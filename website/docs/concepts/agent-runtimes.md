...
- **`spec.execution.workspace` Task is rejected**: workspace-provider-backed RuntimeSession dispatch is fail-closed unless `--agent-sandbox-enabled` and `--acp-workspace-dispatch-enabled` are both set, the provider is `agent-sandbox`, and the request omits `templateRef` and other unsupported options. The rejection reason is projected to `Task.status.executionWorkspace`. Repository access always uses top-level `spec.workspace`; see [Agent Sandbox](agent-sandbox.md) and [Substrate](substrate.md).
