...
- `Live Agent Sandbox E2E` — installs the pinned upstream `agent-sandbox` release in Kind, builds the PR controller, the immutable Codex ACP runtime image, and fixture/router images, then validates the direct workspace-adapter lifecycle (claim, exec, cleanup, retained reuse, token scrubbing) **and** the workspace-backed ACP Task infrastructure smoke: a `Task.spec.execution.workspace` agent Task is admitted, binds a dedicated `acp-ws-*` RuntimePool whose SandboxClaim hosts the real supervisor, reaches authenticated `Serving`, keeps Task status provider-neutral, and cleans up on pool deletion — all without model access (prompt-level completion is covered by the live ACP runtime workflows).
...
- `Agent Substrate E2E` — builds the PR controller, the immutable Codex ACP runtime image, and Substrate fixture images on a gVisor Kind cluster, validates the direct Substrate actor/router/daemon lifecycle and Substrate-backed MCP Tools, and runs the workspace-backed ACP Task infrastructure smoke: a `provider: substrate` Task binds an `acp-ws-*` RuntimePool, the controller renders a derived ActorTemplate from the operator infrastructure template, the supervisor boots inside a gVisor Actor, and the authenticated fence probe reaches `Serving` through the atenet-router. It is secret- and model-free; prompt-level completion and clean-room publication remain live-ACP-workflow coverage.
...
The agent-sandbox and Substrate scripts exercise the earlier execution-workspace prototype. They are not ACP v2 release evidence. Workspace-provider-backed ACP dispatch is flag-gated behind `--acp-workspace-dispatch-enabled` plus the matching provider flag (`--agent-sandbox-enabled` or `--substrate-enabled`) and fails closed otherwise; live workspace-backed Task E2E promotion is tracked by #343.
...
non-secret standard OTLP environment. ACP RuntimePool telemetry is explicit
opt-in on the managed runtime workload; OTLP endpoint variables alone do not
enable runtime telemetry. Delegated child Tasks continue the
