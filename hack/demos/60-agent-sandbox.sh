...
demo_scenario "Agent Sandbox — multi-turn workspace reuse" \
  "Three sequential agent Tasks reattach to ONE workspace via sessionRef. Heavy setup (git checkout, deps, runtime, planning notes) is paid once; turns 2 and 3 start hot."
...
demo_event "📥" "Two Agent CRs (different tool allowlists) and a session name. The workspace is created on demand by the first Task."
...
demo_event "1️⃣ " "Turn 1 = scout. sessionRef.create=true mints a new SandboxClaim for this session."
...
demo_event "2️⃣ " "Turn 2 = builder. sessionRef.create=false attaches the EXISTING claim. Cold-start cost: zero."
...
demo_event "3️⃣ " "Turn 3 = CI fixup. Branch from turn 2 is STILL CHECKED OUT — no re-clone, no context loss."
...
demo_event "🔍" "Payoff card extracts the claim name from each turn's logs and ASSERTS they all match. Demo fails if not."
